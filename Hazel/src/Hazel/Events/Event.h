...
namespace Hazel
{
	/* Events in Hazel are currently blocking, meaning when an event occurs it immediately gets
...
//----------------------------------------------------------------------------------------------------------------------
	...
	public:
		...
		template<typename T>
		bool Dispatch(const EventFn<T> function)
		{
			// static_asset(std::is_based_of_v<Event, T>, "T must inherit from Event!!")
